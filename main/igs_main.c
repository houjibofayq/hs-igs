...
static uint8_t ixe_app_set_wifi(uint8_t *data)
{
  ...
  switch(OnOff){
        ...
        case 0x00:
			      //x_datas.restart = 0x01;
				  xmqtt_set_status(XMQTT_RESTART);
	              change_flag = 1;
				  break;
		case 0x01:
...
void app_main(void)
{
	...
	xTaskCreate(ixe_mqtt_task, "mqtt_sendrecv_data_task", 4096, NULL, 4, NULL); 
	xTaskCreate(ixe_ota_task, "ixe_ota_task", 4096, NULL, 3, NULL);
	
	#ifdef PRODUCT_IGS
	xTaskCreate(igs_util_task, "ise_util_pump_task",2048, NULL, 10, NULL);
	#endif
